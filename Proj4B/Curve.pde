...
class Curve {
  int n=0;  
   int sub_n;  // current number of control points
...
// ******************************************************************************************** LACING ***************
  ...
  void resample2(int numPt){
      ...
      int sub_n=0;
       ...
       float len= sub_n/(float)numPt;
       for (int i=0;i<numPt;i++){
         P[i]=subP[round(i*len)];
       }
       P[numPt-1]=subP[sub_n-1];//last point preserved 
       n=numPt;
 ...
 //**** subdivide
   void subdivide(){
      //System.out.println("nv: "+nv);//for testing
   ...
   //   println(n);
      if(n<500){
       ...
       // System.out.println(sub_nv+"  "+nv);//for testing
    
        subP[0]=P[0]; sub_n++;
        subP[1]=NevilCurve( P[0], P[1], P[2], 0.5); sub_n++;
        for (int i=0;i<n-3;i=i+1){
            subP[sub_n]=P[i+1];
            sub_n++;
            subP[sub_n]= subdiv(P[i],P[i+1],P[i+2],P[i+3]);
            sub_n++;
        }
        
        subP[sub_n]=P[n-2]; sub_n++;
        subP[sub_n]=NevilCurve(P[n-3],P[n-2], P[n-1],1.5); sub_n++;
        subP[sub_n]=P[n-1]; sub_n++;
